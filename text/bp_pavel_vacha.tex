...
\chapter{Úvod}
Vzorce spotřeby a jejich dopady na naše životní prostředí jsou aktuálně mezi největšími výzvami naší doby. Díky pochopení jak a kdy lidé spotřebovávají energii a jak se tyto vzorce chování mění, je možné zajistit udržitelnou budoucnost. Tento úkol se stává ještě naléhavějším v kontextu kolísajících cen energií na trhu, které mohou poskytnout příležitosti k ekonomickým úsporám a optimalizaci energetických systémů.
...
V této práci si autor klade za cíl vyvinout model lokální spotřeby založený na historických datech spolu s environmentálními parametry, pokud jsou pro danou oblast dostupné. Tento cíl vychází z potřeb a požadavků zákazníků, společnosti Albistech s.r.o., kteří vyjádřili zájem o využití kolísajících cen energií na trhu ve svůj prospěch.


Využití kolísání cen energií na trhu může přinést zákazníkům významné ekonomické úspory. Tato práce se proto snaží přispět k pochopení problematiky predikce spotřeby energie a k vytvoření přesných modelů, které budou základem pro řízení energetických systémů v této dynamické a neustále se měnící oblasti.

Provedená rešerše existujících metod prediktivního modelování poskytne základní přehled o možných přístupech k vytvoření robustního a spolehlivého modelu lokální spotřeby. Klasické statistické metody, jako je ARIMA, budou porovnány s moderními technikami strojového učení, jako jsou rozhodovací stromy nebo rekurentní neuronové sítě.
...
\end{equation}
kde $x_t$ je hodnota v čase $t$ a $x_{t-1}$ je vzorek předcházející. Při analýze následující řady (Obrázek \ref{fig:nezamestnanost}) níže, kde je vidět lineární rostoucí trend, je třeba provést diferenci, aby tento trend zmizel.
...
Pro dekompozici se zpravidla používají dva základní modely reprezentující typ časové řady, a to aditivní a multiplikativní. Volba modelu závisí na charakteru sezónnosti a trendu v časové řadě.

\subsection{Aditivní model}
Pro tento model se předpokládá, že řadu lze rozložit pomocí dalších metod na součet jednotlivých komponent.
...
Analogicky tento model naopak předpokládá, že řadu lze rozložit pomocí dalších metod na součin jednotlivých komponent.
\begin{equation}
	x_t = T_t * S_t * C_t * \epsilon_t
\end{equation}
...
\begin{figure}[htbp]
	...
	\label{fig:my_graph_klouzej}
\end{figure}

Na průběhu grafu \ref{fig:my_graph_klouzej} výše lze vidět, že klouzavý průměr (červená čára) s řádem $m=30$ vyhladil průběh časové řady od výkyvů a zvýraznil trend-cyklickou komponentu.
...
Z pohledu na průběh grafu \ref{fig:stl_decoomp} výše je vidět, že řada má nějaký trend i sezónnost. Na grafu náhodné komponenty lze zaznamenat několik vysokých výkyvů. V roce 2008 náhodná komponenta ukázala výrazný nárůst nezaměstnanosti, který nelze vysvětlit trendem, ani cyklickou složkou. (V tomto roce začala tzv. Velká recese).
...
%No to asi ne%
\chapter{Průzkum současných prediktivních metod}
...
Nakonec budou tyto modely podrobeny důkladnému hodnocení založenému na metrikách přesnosti, které jsou uvedeny v kapitole \ref{section:error-matrix}. Vyhodnocení na základě těchto metrik bude klíčovým krokem při posuzování celkové úspěšnosti modelů a pomůže identifikovat případné oblasti, které vyžadují zdokonalení. Vyhodnocení povede také k informovanému rozhodování o jejich budoucí integraci v informačním systému NEO. Modely budou natrénovány na agregované datové sadě a porovnány na stejných časových úsecích náhodně vybraných domácností pro objektivní zhodnocení. Predikované časové úseky budou v krátkodobém horizontu jednoho dne a týdne.
...
Na základě této provedené analýzy \ref{fig:hodina_quarters} lze zařadit parametr \textit{quarter} jakožto indikátor ročního období mezi další příznaky, jenž bude model pro predikce využívat.
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.8\textwidth]{pv_eda_02_weather_over_consumption.eps}}%
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.6\textwidth]{pv_eda_02_heat_weather.eps}}%
...
Gradientní boostované rozhodovací stromy lze snadno analyzovat a interpretovat díky jejich \textit{ensemble} architektuře. Proto je možné jednoduše zjistit jak moc důležité jsou jednotlivé parametry pro rozhodování natrénovaného modelu. Pro výpočet skóre důležitosti příznaků byla využita diagnostická funkce \textit{plot\_importance}, kterou poskytuje knihovna XGBoost v Pythonu. Toto skóre udává, jak moc každá proměnná přispívá k rozhodování modelu a napomáhá k vyřazení nepotřebných příznaků za účelem optimalizace a zvýšení přesnosti. 
 ...
 \begin{figure}[htbp]
 	\centering
 	\makebox[\textwidth][c]{\includegraphics[width=0.6\textwidth]{xgboost_f_score.eps}}%
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.6\textwidth]{xgboost_learning_curve.eps}}%
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.9\textwidth]{xgboost_predictions.eps}}%
...
\begin{figure}[htbp]
 	\centering
 	\makebox[\textwidth][c]{\includegraphics[width=0.8\textwidth]{arima_pacf_acf.eps}}%
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.6\textwidth]{cnn_lstm_learning_curve.eps}}%
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.9\textwidth]{lstm_vyrez.eps}}%
...
\begin{table}[!ht]
	...
	\begin{tabular}{lllllll}
		...
		~               & Brno, byt 5.02              & 0.001            & 0.030             & 0.022           & 48.032 \\         
		...
		~               & Brno, byt 5.02              & 0.004            & 0.059             & 0.044           &	5.512\\         
		...
		~               & Brno, byt 5.02  & 0.001          & 0.015             & 0.001            & 16.838\\   
		...
		~               & Brno, byt 5.02  & 0.002           & 0.039             & 0.029           & 48.479 \\   
...
Toto zjištění vychází z grafické inspekce dat, kde bylo zjištěno, že ARIMA model má potíže reagovat na rychlé výkyvy ve spotřebě, jak je znázorněno na grafu z obrázku \ref{fig:spike_analysis} níže.

\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.8\textwidth]{spike.eps}}%
...
\begin{table}[!ht]
	...
	\begin{tabular}{lllllll}
		...
		~               & Brno, byt 5.02              & 0.060            & 0.024             & 0.019           & 36.892 \\         
		...
		~               & MAC004385          & 0.010            & 0.065             & 0.071           & 11.352            \\ 
		~               & Brno, byt 5.02              & 0.004            & 0.060             & 0.046           & 5.331 \\         
		...
		~               & Brno, byt 5.02  & 0.001           & 0.016             & 0.015            & 13.781\\   
		...
		~               & Brno, byt 5.02              & 0.001           & 0.037             & 0.026           & 37.800 \\   
...
\section{Průběhy XGBoost}

\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{apendix/xgboost/prediction_82293.eps}}%
	\caption{Předpověď 336 hodnot - 16.84 \% MAPE}
	\label{fig:lstm_vyrez}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.1\textwidth]{apendix/xgboost/prediction_1_86.eps}}%
	\caption{Předpověď 48 hodnot - 1.86 \% MAPE}
	\label{fig:lstm_vyrez}
\end{figure}

\newpage
\section {Průběhy ARIMA}

\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.95\textwidth]{apendix/arima/pred_1.eps}}%
	\caption{Předpověď 24 hodnot - 79.36 \% MAPE}
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=0.95\textwidth]{apendix/arima/spike.eps}}%
	\caption{Předpověď 168 hodnot - 39.15 \% MAPE}

\end{figure}

\newpage
\section{Průběhy LSTM}
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{apendix/lstm/lstm_28_25.eps}}%
	\caption{Předpověď 336 hodnot - 28.25 \% MAPE}
	\label{fig:lstm_vyrez}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{apendix/lstm/lstm_26_24.eps}}%
	\caption{Předpověď 48 hodnot - 26.24 \% MAPE}
	\label{fig:lstm_vyrez}
\end{figure}


\newpage
\section{Průběhy CNN-LSTM}
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{apendix/lstm/cnn_15_78.eps}}%
	\caption{Předpověď 48 hodnot - 15.78 \% MAPE}
	\label{fig:lstm_vyrez}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{apendix/lstm/cnn_24_60.eps}}%
	\caption{Předpověď 336 hodnot - 24.60 \% MAPE}
	\label{fig:lstm_vyrez}
\end{figure}
